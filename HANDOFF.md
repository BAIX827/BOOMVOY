...
- 2026-09-08 Boomi 更新：`boomiGuide.ts` 优先选当前打开的旅行，再选现有非模板旅行；有旅行采用 6 步核心引导，无旅行采用 4 步创建 / 模板引导，不依赖 Japan 2026。快捷问题随页面变化；本地回答提供明确的“打开对应页面”按钮，不自动跳走。
- `boomiVoice.mjs` 统一处理欢迎语、页面提示、引导、本地回答、等待提示与 AI 回复，非空正文结尾只保留一个“喵”。中英文正文同步，标题、按钮与用户消息不加后缀。服务端同样归一 AI 回答并约束为简短、可执行、符合现有能力的说明。
- 对话支持关闭、Esc、引导键盘焦点限制和清空。关闭 / 换页 / 换语言时取消当前回复的等待并隔离旧结果；首次欢迎与高亮定时器不会在关闭后重新弹出。底层仍保留共享请求合并。
...
- Japan 2026 是内置的 10 天日本演示旅行，包含东京、富士、京都、大阪的示例行程、预算和预订记录，订单不代表真实购买。它不是应用运行所必需；本轮保留现有数据，仅去掉引导对其 ID 的硬编码依赖。用户可在该旅行的“笔记”页删除；恢复示例会替换现有旅行数据，不能当作追加导入。
